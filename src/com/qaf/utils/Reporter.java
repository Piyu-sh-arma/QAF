...
public class Reporter {
        ...
        if (FilesUtil.fileExist(sourceFile)) {
            try {
                ...
                if (null == testDetails) {
                    ...
                    testName = testDetails.get("Test Name");
                    almTestId = testDetails.get("ALM ID");
                }
                ...
                String resultFileHeaderPart2 = "\n<script>\ndocument.getElementById('testName').innerText = '" + testName + "'\n"
                        ...
                        + "document.getElementById('ALMID').innerText = '" + almTestId + "'\n" + "document.getElementById('Starttime').innerText = '"
    ...
    public static void reportStep(String step, String details, StepStatus status) {
        ...
        try {
            ...
            logger.info("Step - " + step + ", Details - " + details + ", Status - " + status.toString());
        } catch (IOException e) {
            // TODO Auto-generated catch block
            e.printStackTrace();
        }
    ...
    public static void closeReport() {
        try {
            String tstRunKey = Thread.currentThread().getName();
            String tstKey = tstRunKey.split("~_")[1];
            logger.info("Closing report for test " + tstKey);
            ...
            logger.info("Closed report - " + curTestRunInfo.resultFilePath);
        } catch (Exception e) {
